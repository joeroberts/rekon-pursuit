...
| `UX-R2` | Pipeline-owned core workflow forms: Add Opportunity layout/validation/structured action details and creation-date default; contact validation/employer association/expandable text; and staged CSV mapping/review/completion redesign. | `UX-R1` | **In progress — UX-R2-B released** | Full UX-R2 acceptance still requires B (Contacts), C (CSV), and product-owner workflow verification. | Planning, Architect, QA, TPM, and Delivery approved the brief. Product-owner acceptance closed A; Delivery released B only. C and RP-R6 remain unreleased. |
