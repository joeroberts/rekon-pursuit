...
progress: its `RP-R7a-3` restore-as-new-workspace work remains active.
`RP-R7a-3a` restore-worker foundation is Accepted. `RP-R7a-3b` restore
UI/owner smoke is released and In progress under its separately approved
scope.
...
| `RP-R7a` | Recovery-key enrollment/verification, authenticated portable archive/export default, minimal creation/expiry/recoverability and retained-deletion disclosure, and restore-as-new-workspace. | `RP-R6` | **In progress — R7a-1, R7a-2, and R7a-3a accepted; R7a-3b released** | Recovery-key, portable restore, encrypted/default-export, and truthful backup-state evidence. | Recovery-key setup, archive creation/cataloguing, and the non-UI restore foundation are accepted. The R7a-3 parent remains active. Restore UI/owner smoke is in progress; export, purge, and expiry removal remain unreleased. |
| `RP-R7a-3` | Verify a portable archive and import it into a fresh separate local workspace; do not activate/switch it. | `RP-R7a-2` | **In progress — R7a-3a accepted; R7a-3b released** | Typed package/snapshot verification; fresh-key target; document relink-required; non-destructive candidate lifecycle. | The worker foundation is accepted. Only the separately gated R7a-3b UI adapter is released. No activation/switch, export, purge, expiry, deletion, or legacy-route work is authorized. |
...
| `RP-R7a-3b` | Restore UI adapter, recovery-key entry, confirmation/progress surfaces, and owner smoke. | `RP-R7a-3a` accepted plus its own plan and gates | **In progress — released** | Its approved plan, focused UI/core boundary evidence, independent implementation review, and hands-on smoke. | Planning, Architect/Security, QA, TPM, and Delivery approved the direct scoped-file and all-restores confirmation scope. No candidate open/activation/switch, archive-creation changes, export, purge, expiry, deletion, network, or legacy route is released. |
