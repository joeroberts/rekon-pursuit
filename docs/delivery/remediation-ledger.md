...
Accepted; `UX-R2` is In progress on **UX-R2-A only** (opportunity
capture/overview compatibility and validation); `RP-R6` and `RP-R7a`–`RP-R10`
are Backlog. Contacts and CSV redesign remain unreleased.
...
| `UX-R2` | Pipeline-owned core workflow forms: Add Opportunity layout/validation/structured action details and creation-date default; contact validation/employer association/expandable text; and staged CSV mapping/review/completion redesign. | `UX-R1` | **In progress — UX-R2-A ready for hands-on review** | Opportunity Add/Overview compatibility data, validation, focused persistence tests, and product-owner hands-on review. | Planning, Architect, QA, TPM, and Delivery approved the brief. UX-R2-A implementation plus corrective independent reviews are complete; only B (Contacts), C (CSV), and RP-R6 remain unreleased. |
