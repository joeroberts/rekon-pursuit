...
Accepted; `UX-R2` is Next up for its focused plan/release gate; `RP-R6` and
...
| `UX-R2` | Pipeline-owned core workflow forms: Add Opportunity layout/validation/structured action details and creation-date default; contact validation/employer association/expandable text; and staged CSV mapping/review/completion redesign. | `UX-R1` | Next up | Focused workflow/form verification and product-owner hands-on review. | Eligible for its own focused plan/release gate; no implementation released. |
