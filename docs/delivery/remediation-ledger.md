...
| `RP-R1a` | Typed workspace gate: visible create/open/recover/retry states, including dangling primary/pending Keychain key without database; no overwrite or automatic key deletion; CSV availability follows ready state. Creation uses explicit pre-commit/committed phases and never deletes a committed database on an open error. | `RP-R0` | **Released — active; plan amendment review required** | Fresh-launch runtime smoke uses a distinct sandboxed temporary bundle, app-data container, and Keychain service: create a disposable workspace, CSV entry becomes selectable, fixture preview appears, visible state refreshes without relaunch, and retry/error paths preserve existing artifacts. This task does not accept visual shell alignment. | Security/Privacy P1s require Architect, TPM, QA, and Delivery approval of this amendment before implementation; no successor is released. |
