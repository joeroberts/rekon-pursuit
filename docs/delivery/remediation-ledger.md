...
Accepted; `UX-R2-A` is Accepted and parent `UX-R2` remains In progress;
`UX-R2-B` (Contacts) is Next up but unreleased. `UX-R2-C` (CSV redesign),
...
| `UX-R2` | Pipeline-owned core workflow forms: Add Opportunity layout/validation/structured action details and creation-date default; contact validation/employer association/expandable text; and staged CSV mapping/review/completion redesign. | `UX-R1` | **In progress — UX-R2-A accepted; UX-R2-B Next up** | Full UX-R2 acceptance still requires B (Contacts), C (CSV), and product-owner workflow verification. | Planning, Architect, QA, TPM, and Delivery approved the brief. Product-owner acceptance closed A only. B is the sole eligible successor but has not been released; C and RP-R6 remain unreleased. |
...
| Delivery Manager | **Accepted — sub-slice complete** | Acceptance closes only UX-R2-A. Move UX-R2-B to Next up; do not release B implementation. UX-R2-C and RP-R6 remain unreleased. |
