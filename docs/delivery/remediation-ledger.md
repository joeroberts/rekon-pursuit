...
UI/owner smoke is Next up for its own planning and approval gates; it remains
unreleased.
...
| `RP-R7a` | Recovery-key enrollment/verification, authenticated portable archive/export default, minimal creation/expiry/recoverability and retained-deletion disclosure, and restore-as-new-workspace. | `RP-R6` | **In progress — R7a-1, R7a-2, and R7a-3a accepted** | Recovery-key, portable restore, encrypted/default-export, and truthful backup-state evidence. | Recovery-key setup, archive creation/cataloguing, and the non-UI restore foundation are accepted. The R7a-3 parent remains active. Restore UI/owner smoke is Next up for its own gates; export, purge, and expiry removal remain unreleased. |
| `RP-R7a-3` | Verify a portable archive and import it into a fresh separate local workspace; do not activate/switch it. | `RP-R7a-2` | **In progress — R7a-3a accepted; R7a-3b Next up** | Typed package/snapshot verification; fresh-key target; document relink-required; non-destructive candidate lifecycle. | The worker foundation is accepted. No switch, export, purge, expiry, deletion, legacy-route, or UI work is released; R7a-3b must complete its own planning and approval gates before release. |
...
| `RP-R7a-3b` | Restore UI adapter, recovery-key entry, confirmation/progress surfaces, and owner smoke. | `RP-R7a-3a` accepted plus its own plan and gates | **Next up — planning/gates required** | Its own plan, Architecture/Security, QA, TPM, Delivery approval, and hands-on evidence. | Not blocked: it is ordinary sequencing. It has not been released and no implementation starts until its own gates approve the UI scope. |
