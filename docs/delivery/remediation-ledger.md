...
is Blocked on a stable development-signing identity needed for sandboxed
Keychain access; `UX-R2`, `RP-R6`, and `RP-R7a`–`RP-R10` are Backlog. No
successor implementation is released.
...
| `UX-R1` | Shell and opportunity navigation: native icon/compact sidebar emblem, first-run onboarding, Home default with Needs Attention as its first section, Pipeline-owned add/import entry points, consistent Rekon tokens, responsive empty states, scrollable Pipeline → dedicated opportunity overview, Activity & history and Reconcile posting sub-screens, and a compact overview document area. | `RP-R5` | **Blocked** | Stable development signing identity, sandboxed Keychain-open proof against the preserved workspace, then focused navigation/layout/state verification and product-owner hands-on review. | The current ad-hoc-signed sandboxed handoff cannot access the existing app Keychain key and reports a false recovery state. The workspace database and primary key are present; nothing was replaced. Configure a stable development-signing identity before another sandboxed handoff. UX-R2 and R6 remain unreleased. |
...
| `UX-RISK-1` | A sandboxed app signed with an identity-free ad-hoc signature cannot reliably retrieve the app-scoped Keychain key across handoff rebuilds. The recovery screen is therefore false for a preserved workspace. Do not delete, recreate, or overwrite its database/Keychain material. | Product owner + Architect | **Blocked** — configure a stable Apple development-signing identity / Personal Team, then prove a sandboxed signed build opens the existing workspace. A migration of existing ad-hoc Keychain material, if ever needed, requires separate Architect + Security/Privacy approval. |
