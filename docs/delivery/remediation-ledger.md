...
Accepted. The wider `RP-R7a` recovery milestone remains in progress because
its encrypted-default export work has not yet been planned or released.
Export, purge, and expiry remain unreleased; there is no active implementation
task until export receives its own bounded plan and gates.
...
| `RP-R7a` | Recovery-key enrollment/verification, authenticated portable archive/export default, minimal creation/expiry/recoverability and retained-deletion disclosure, and restore-as-new-workspace. | `RP-R6` | **In progress — recovery and restore accepted; export unplanned** | Recovery-key, portable restore, encrypted/default-export, and truthful backup-state evidence. | Recovery-key setup, archive creation/cataloguing, and restore-as-new-workspace are accepted. Encrypted-default export needs its own plan and release gate; purge and expiry removal remain unreleased. |
