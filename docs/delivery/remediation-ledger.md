...
| `RP-R2` | Migration plus editable compensation, location, response state/history, and application/status dates. | `RP-R1b` | **Released — active** | Existing-workspace migration and create/edit/relaunch activity evidence. | R1b accepted; R2 is the only dependency-safe successor. `RP-R3`–`RP-R10` remain blocked. |
