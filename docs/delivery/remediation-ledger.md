...
| `UX-R2` | Pipeline-owned core workflow forms: Add Opportunity layout/validation/structured action details and creation-date default; contact validation/employer association/expandable text; and staged CSV mapping/review/completion redesign. | `UX-R1` | **In progress — UX-R2-A only** | Opportunity Add/Overview compatibility data, validation, focused persistence tests, and product-owner hands-on review. | Planning, Architect, QA, TPM, and Delivery approved the brief. Only UX-R2-A is released; B (Contacts), C (CSV), and RP-R6 remain unreleased. |
