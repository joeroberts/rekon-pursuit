...
Accepted. The wider `RP-R7a` recovery milestone remains in progress. Its
`RP-R7a-4` encrypted-default export task is Next up for planning and release
gates; implementation is not released. Purge and expiry remain unreleased.
...
| `RP-R7a` | Recovery-key enrollment/verification, authenticated portable archive/export default, minimal creation/expiry/recoverability and retained-deletion disclosure, and restore-as-new-workspace. | `RP-R6` | **In progress — recovery/restore accepted; R7a-4 Next up for gates** | Recovery-key, portable restore, encrypted/default-export, and truthful backup-state evidence. | Recovery-key setup, archive creation/cataloguing, and restore-as-new-workspace are accepted. R7a-4 plans the separate encrypted-default export boundary; implementation, purge, and expiry removal remain unreleased. |
...
| `RP-R7a-4` | One encrypted-default `.rekonexport` logical export of the fixed Tracker workspace data category, protected with operation-time recovery-key re-entry and final review. | `RP-R7a-3` accepted | **Next up — planning and release gates** | Versioned container, domain-separated encryption, final-review binding, destination safety, redaction, and product-owner smoke evidence. | Planning, Architecture/Security, and TPM approved the bounded release-gate stage. QA and Delivery review, plus frozen container framing/review binding, remain before implementation can be released. |
...
| Architect / Security | **Approved with implementation freeze note** | Reuse the user-held recovery key only in operation memory with a domain-separated, per-export wrapping key. Before implementation release, freeze the canonical container framing and mechanically bind/recheck source revision and canonical destination. |
...
| QA | Pending | Independent review in progress; no implementation is released. |
| Delivery Manager | Pending | May release only after the remaining QA and technical-freeze conditions are recorded. |
