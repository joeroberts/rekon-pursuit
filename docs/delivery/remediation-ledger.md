...
| `RP-R1a` | Typed workspace gate: visible create/open/recover/retry states, including the currently unmodeled dangling-Keychain-key/no-database condition; no overwrite; CSV availability follows ready state. | `RP-R0` | Blocked | Fresh-launch runtime smoke: create a disposable workspace, CSV entry becomes selectable, fixture preview appears, visible state refreshes without relaunch, and retry/error paths are understandable. This task does not accept visual shell alignment. | Pending accepted `RP-R0`. |
