...
is Blocked on a material storage-access decision for the preserved legacy
workspace; `UX-R2`, `RP-R6`, and
...
| `UX-R1` | Shell and opportunity navigation: native icon/compact sidebar emblem, first-run onboarding, Home default with Needs Attention as its first section, Pipeline-owned add/import entry points, consistent Rekon tokens, responsive empty states, scrollable Pipeline → dedicated opportunity overview, Activity & history and Reconcile posting sub-screens, and a compact overview document area. | `RP-R5` | **Blocked** | Product-owner storage-access decision; then revised Architect/Security plan, signed synthetic cross-build Keychain proof, true read-only workspace verification, focused navigation/layout/state verification, and product-owner hands-on review. | Stable Personal-Team signing is verified. Independent review established that the sandbox lacks write access to the preserved external workspace and the former verifier would mutate WAL state. No key/database migration or deletion has run. UX-R2 and R6 remain unreleased. |
...
### UX-R1 legacy-workspace corrective review

| Role | Decision | Evidence / required correction |
| --- | --- | --- |
| Planning | Approved the bounded corrective scope | The original plan limited work to an in-memory Keychain handoff and retained the source database and legacy key. |
| Architect / Security | **Not released — P1 corrections** | The existing open API is read/write and enables WAL; a future verifier must use `SQLITE_OPEN_READONLY`. Entitlement comparison is not enough; use a signed cross-build synthetic Keychain proof. |
| QA / Security | **Blocked — P0 storage boundary** | The normal sandbox resolves Application Support inside its container and has no persistent writable permission to the preserved external workspace. A Keychain-only handoff cannot open it. |
| TPM / Delivery | **Blocked** | `UX-R1` remains the active blocked umbrella; UX-R2 and R6 remain Backlog. The previous user approval is recorded, but a new storage model is a material decision outside that approval. |
| Product owner | Pending | Choose persistent user-selected read/write folder access without copying (recommended) or approve a separately designed, verified copy into the sandbox that retains the original source. |
...
| `UX-RISK-1` | Stable Personal-Team signing is configured and verified, but the sandboxed app cannot access the legacy key or write to the preserved external workspace. The recovery screen is safe but cannot resolve either boundary. Do not delete, recreate, overwrite, extract, or log the database/Keychain material. | Product owner + Architect + Security/Privacy | **Blocked** — select persistent user-selected read/write folder access (recommended, no copy) or approve a separately designed verified-copy migration that preserves the original source. |
