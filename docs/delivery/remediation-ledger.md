...
`RP-R1a`, `RP-R1b`, `RP-R2`, `RP-R3`, `RP-R4`, and `RP-R5` Accepted; `RP-R6`
is Next up; `RP-R7a`–`RP-R10` are Backlog; and no task is Blocked. R6 has not
...
| 2026-07-26 | `RP-R5` | **Accepted — R6 Next up** | Corrected implementation reviews and product-owner hands-on verification are complete. Move R5 to Accepted and R6 to Next up only; R6 needs its own plan and release gate before implementation. |
...
| `UX-D1` | Complete the Rekon Pursuit brand pass: a native macOS app icon derived from the target/arrow emblem, plus a compact in-app emblem/wordmark. The placeholder app icon and plain title pill are not acceptable. | Product owner | Accepted 2026-07-26; queued for the UX remediation pass before R6 implementation. |
| `UX-D2` | Workspace creation belongs only to dedicated first-run onboarding. Once a workspace exists, daily views must not repeat a workspace card; workspace health, backup, recovery, and privacy details belong in Settings. | Product owner | Accepted 2026-07-26; queued for the UX remediation pass before R6 implementation. |
| `UX-D3` | Pipeline is a scrollable list/board. Selecting an opportunity opens a dedicated detail view that returns to the prior list state; do not use a permanent below-list editor or floating pop-up card. | Product owner | Accepted 2026-07-26; queued for the UX remediation pass before R6 implementation. |
