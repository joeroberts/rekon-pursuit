...
is Blocked on an approved one-time migration of the legacy ad-hoc Keychain
material into the stable sandboxed app scope; `UX-R2`, `RP-R6`, and
...
| `UX-R1` | Shell and opportunity navigation: native icon/compact sidebar emblem, first-run onboarding, Home default with Needs Attention as its first section, Pipeline-owned add/import entry points, consistent Rekon tokens, responsive empty states, scrollable Pipeline → dedicated opportunity overview, Activity & history and Reconcile posting sub-screens, and a compact overview document area. | `RP-R5` | **Blocked** | Architect- and Security/Privacy-approved one-time legacy-Keychain migration design, then sandboxed Keychain-open proof against the preserved workspace, focused navigation/layout/state verification, and product-owner hands-on review. | Stable Personal-Team signing is verified, but the sandboxed app cannot access the legacy key created by the former ad-hoc identity. The database remains intact and no key/database migration or deletion has run. UX-R2 and R6 remain unreleased. |
...
| `UX-RISK-1` | Stable Personal-Team signing is now configured and verified, but it correctly cannot retrieve the workspace key created by the earlier identity-free ad-hoc app. The recovery screen is safe but cannot resolve this authorization boundary. Do not delete, recreate, overwrite, extract, or log the database/Keychain material. | Product owner + Architect + Security/Privacy | **Blocked** — approve/release a narrowly scoped one-time local migration design that verifies the legacy key and database, adds only the new app-scoped Keychain material, proves the signed app can open it, and retains original artifacts until explicit acceptance. |
