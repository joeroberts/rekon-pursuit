...
`UX-R2-C` (CSV redesign) is Next up but unreleased.
...
| `UX-R2` | Pipeline-owned core workflow forms: Add Opportunity layout/validation/structured action details and creation-date default; contact validation/employer association/expandable text; and staged CSV mapping/review/completion redesign. | `UX-R1` | **In progress — UX-R2-C next up** | Full UX-R2 acceptance still requires C (CSV) and product-owner workflow verification. | Planning, Architect, QA, TPM, and Delivery approved the brief. Product-owner acceptance closed A and B; C is the only eligible successor. RP-R6 remains unreleased. |
