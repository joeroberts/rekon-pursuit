...
`RP-R7a-4` encrypted-default export task is In progress as the only released
implementation task. Purge and expiry remain unreleased.
...
| `RP-R7a` | Recovery-key enrollment/verification, authenticated portable archive/export default, minimal creation/expiry/recoverability and retained-deletion disclosure, and restore-as-new-workspace. | `RP-R6` | **In progress — recovery/restore accepted; R7a-4 implementation active** | Recovery-key, portable restore, encrypted/default-export, and truthful backup-state evidence. | Recovery-key setup, archive creation/cataloguing, and restore-as-new-workspace are accepted. R7a-4 is the only released encrypted-default export implementation; purge and expiry removal remain unreleased. |
...
| `RP-R7a-4` | One encrypted-default `.rekonexport` logical export of the fixed Tracker workspace data category, protected with operation-time recovery-key re-entry and final review. | `RP-R7a-3` accepted | **In progress — implementation released** | Versioned container, domain-separated encryption, final-review binding, destination safety, redaction, and product-owner smoke evidence. | Planning, Architecture/Security, TPM, QA, and Delivery approved the frozen encrypted-only contract. Implement only this export; no successor, unencrypted/CSV, archive/restore, purge/expiry, deletion, network, AI, or dependency expansion is released. |
