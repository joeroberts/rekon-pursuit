...
After re-gates, writable paths are only `PipelineView.swift`, `PipelineBoardView.swift`, `ContentView.swift`, `WorkspaceViewModel.swift` for pure reset, `RekonPursuitTests.swift`, and `RekonPursuitUITests.swift`. In Task 2, `PipelineBoardView.swift` is writable solely to delete the obsolete private `dropTarget`; the existing delegates then consume the canonical exact property, intentionally changing Applied drops to `.applied`. Lane-container rendering, menu, scroll, and focus changes remain reserved for Task 3.
