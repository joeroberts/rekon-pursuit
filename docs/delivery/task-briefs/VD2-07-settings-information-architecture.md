...
| `RekonPursuitUITests/RekonPursuitUITests.swift` | Add the deterministic fixture-driven Settings navigation, compact/keyboard, recovery/archive root-modal cancellation, document-redaction, AI-unavailable, active-workspace relaunch, and global-rail regression selectors described below. |
| `RekonPursuitTests/WorkspaceViewModelTests.swift` | Add one non-UI cancellation regression: after a reviewed protected export exists, `cancelProtectedExport()` clears the review without writing an output or changing the active workspace facts. The test generates its recovery key only in memory and never records its display value. |
...
`WorkspaceViewModel.swift`, `RekonPursuitCore/**`, migrations, persistence, test-host launch parsing, fixture identities, app routes, app-shell navigation, and all recovery/export/archive/document/AI behavior are read-only for this task. The two listed test files may gain only the named assertions; no fixture or production recovery mechanism is introduced to make presentation testing easier.
...
| --- | --- |
| Local selection, rail, compact keyboard, and non-persisted default after relaunch | `testVD207SettingsSecondaryNavigationDefaultsToRecoveryAndKeepsGlobalRail`, `testVD207SettingsSecondaryNavigationIsKeyboardOperableAtCompactWidth`, and `testVD207SettingsRelaunchKeepsFixtureTruthAndResetsLocalSelection`. |
| Enrolled archive truth, fixed date/expiry/lifecycle, retained action IDs/predicates, and root-owned non-secret cancel paths | `testVD207SettingsRecoveryRetainsArchiveTruthAndRootOwnedCancellation` plus `testVD207SettingsRecoveryPresentationPreservesBusyDisabledAndInactiveCandidateContracts`. |
| Protected-export review/cancel/no-write and unchanged active workspace | `testCancellingReviewedProtectedExportClearsReviewWithoutWritingOrChangingActiveWorkspace`, `testReviewBindsDestinationParentIdentity`, `testSourceRevisionChangeRejectsReviewedExportWithoutCreatingAFile`, and `testExistingTargetIsRejectedWithoutOverwritingIt`. |
