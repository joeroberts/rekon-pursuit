...
| `RekonPursuitUITests/RekonPursuitUITests.swift` | Add the deterministic fixture-driven Settings navigation, compact/keyboard, recovery/archive root-modal cancellation and protected-export error-surface, document-redaction, AI-unavailable, active-workspace relaunch, and global-rail regression selectors described below. |
| `RekonPursuitTests/WorkspaceViewModelTests.swift` | Add the reviewed protected-export cancellation regression and one Task-2 root-modal presentation/binding regression. The first proves `cancelProtectedExport()` clears a reviewed export without writing an output or changing active workspace facts; the second proves restore cancellation and failure dismissal clear/cancel/dismiss only the existing root state while the active workspace remains unchanged. The first generates a key only in memory; the second uses only an empty invalid entry. Neither records a display value. |
| `RekonPursuitUITestHostTests/RekonPursuitUITestHostTests.swift` | Add the pure display-state regression for `SettingsRecoveryPresentation` after `SettingsView.swift` exists. It must prove the exact existing disabled predicates, busy text, retained-purge status pass-through, and inactive-candidate text without adding fixture IDs, launch parsing, test-host routes, or product test switches. |

`WorkspaceViewModel.swift`, `RekonPursuitCore/**`, migrations, persistence, test-host launch parsing, fixture identities, app routes, app-shell navigation, and all recovery/export/archive/document/AI behavior are read-only for this task. Only the three test paths in the table may gain the named assertions; no fixture or production recovery mechanism is introduced to make presentation testing easier.

## Required ownership and interfaces

`ContentView` continues to own the existing state currently declared with the Settings view: `generatedRecoveryKey`, `reentry`, `recoveryKeyCopied`, `archiveRecoveryReentry`, `isPresentingArchiveCreation`, `protectedExportReentry`, `isPresentingProtectedExport`, `retainedDataPurgeReentry`, `isPresentingRetainedDataPurge`, and `portableArchiveRestoreKey`. It retains the existing recovery-key enrollment, portable-archive creation, protected-export, retained-data-purge, portable restore, and restore-failure sheets/alert without copy or behavioral changes. Its presentation-only root seam may map the existing restore state and protected-export error into sheet/alert/error visibility and may invoke only the existing clear, cancel, and dismiss closures; it may not retain a model, a key, a fixture value, or persistent state.
...
| --- | --- | --- |
| `populated` | Not enrolled | Default Recovery panel, `set-up-recovery-key`, all four local selectors, exact semantic keyboard-focus/Space selection values, and unchanged selected global Settings rail. |
| `archive` | Enrolled with a verified archive created at the fixed clock | Existing archive/export/purge/restore identifiers; fixed durable filename, created instant, expiry instant, and lifecycle summary; enrolled purge predicate; root-owned archive/export/purge sheet entry and cancellation without typing a key; protected-export empty-entry error surface and cancellation; and non-busy enabled/disabled predicates. |
...
- archive/restore busy, verification failure, cancellation, inactive candidate, and disabled controls: `testPortableArchiveBusyStateRejectsDuplicateAndWorkspaceSwitch`, `testPortableArchiveRestoreKeepsSecurityScopeUntilExplicitConfirmationCompletes`, `testMalformedPortableArchiveRecoveryKeyRemainsVisibleUntilDismissed`, `testPortableArchiveVerificationFailureRemainsVisibleUntilDismissed`, `testCancellingAwaitedPortableArchiveVerificationReleasesScopeExactlyOnceAfterWorkerFinishes`, `testCancellingAwaitedPortableArchiveRestoreReleasesScopeExactlyOnceAfterWorkerFinishes`, and `testPortableArchiveControlsStayDisabledThroughoutAwaitedVerificationAndRestore`;
- protected-export review, cancellation, error, and no-overwrite: `testReviewBindsDestinationParentIdentity`, `testSourceRevisionChangeRejectsReviewedExportWithoutCreatingAFile`, `testExistingTargetIsRejectedWithoutOverwritingIt`, `testProtectedExportReviewFailureRemainsVisibleForCorrection`, and the new `testCancellingReviewedProtectedExportClearsReviewWithoutWritingOrChangingActiveWorkspace`;
...
The exact Task 1 RED and Task 2 GREEN commands must also execute these
already-existing host/UI fixture contracts once each: `testVD202FixtureHostPublishesAProofThatLiveStoresAreUnavailable`, `testVisualFixtureLaunchConfigurationIsExplicitAndIsolated`, `testVisualFixtureConfigurationsUsePerRunTemporaryRoots`, `testVisualFixtureUsesFixedTimeAndReducedMotionContracts`, `testArchiveVisualFixtureConstructionCompletesOnTheMainActor`, `testArchiveVisualFixtureSeedsVerifiedArchiveCatalogue`, `testArchiveVisualFixtureUsesItsDeclaredTimeZoneAtTheCalendarBoundary`, `testDocumentRelinkVisualFixtureContainsASelectedRelinkRequiredReference`, and `testRecoveryFixtureShowsOnlyRecoveryActionsAndDoesNotOpenAWorkspace`. The exact commands and the additional VD2-07 selectors are normative in the implementation plan.

## Presentation safety evidence matrix

The implementation plan must create and run these selectors without typing,
logging, attaching, or screenshotting a recovery-key value:

| Presentation contract | Exact evidence |
| --- | --- |
| Local selection, rail, compact keyboard, and non-persisted default after relaunch | `testVD207SettingsSecondaryNavigationDefaultsToRecoveryAndKeepsGlobalRail`, `testVD207SettingsSecondaryNavigationIsKeyboardOperableAtCompactWidth`, and `testVD207SettingsRelaunchKeepsFixtureTruthAndResetsLocalSelection`. The compact test must observe `Not selected; Keyboard focus` before Space and `Selected; Keyboard focus` after Space. |
| Enrolled archive truth, fixed date/expiry/lifecycle, retained action IDs/predicates, protected-export error surface, and root-owned non-secret cancel paths | `testVD207SettingsRecoveryRetainsArchiveTruthAndRootOwnedCancellation` plus `testVD207SettingsRecoveryPresentationPreservesBusyDisabledAndInactiveCandidateContracts`. |
| Protected-export review/cancel/no-write and unchanged active workspace | `testCancellingReviewedProtectedExportClearsReviewWithoutWritingOrChangingActiveWorkspace`, `testVD207SettingsRootModalBindingsDismissWithoutChangingActiveWorkspace`, `testReviewBindsDestinationParentIdentity`, `testSourceRevisionChangeRejectsReviewedExportWithoutCreatingAFile`, and `testExistingTargetIsRejectedWithoutOverwritingIt`. |
| Retained-purge destructive/cancel safety | `testVD207SettingsRecoveryRetainsArchiveTruthAndRootOwnedCancellation`, `testRetainedDataPurgeRemovesDeletedOpportunityReconciliationAndTombstoneMaterial`, and `testRetainedDataPurgeRejectsWrongKeyBeforeWritingJobOrLease`. |
| Root-owned restore cancellation/failure alert dismissal, disabled state, and inactive candidate | `testVD207SettingsRootModalBindingsDismissWithoutChangingActiveWorkspace`, `testVD207SettingsRecoveryPresentationPreservesBusyDisabledAndInactiveCandidateContracts`, `testPortableArchiveRestoreKeepsSecurityScopeUntilExplicitConfirmationCompletes`, `testPortableArchiveVerificationFailureRemainsVisibleUntilDismissed`, `testCancellingAwaitedPortableArchiveRestoreReleasesScopeExactlyOnceAfterWorkerFinishes`, `testPortableArchiveControlsStayDisabledThroughoutAwaitedVerificationAndRestore`, and `testVerifiedArchiveRestoresIntoANewInactiveWorkspaceWithoutChangingSource`. |
| Document aggregate-only and AI no-control/non-disclosure surfaces | `testVD207SettingsDocumentAndAISectionsStayAggregateAndUnavailable`; its panel-scoped absence checks must inspect descendant labels and values, and all actionable child-control kinds. |
...
Before Task 2 begins, preserve the Task 1 `.xcresult`, fixture sessions, exact selectors, and the current project-file SHA-256. A RED is valid only when the existing signed test host launches a ready named fixture and fails because the new named Settings selector/panel is absent. A build, fixture, signing, test-host, global-rail, or accessibility-query failure is a blocker, not RED evidence. The Task 1 result bundle must show every named pre-existing lower-layer, fixture-host, and recovery-only selector once as passed with no skip, and every new UI selector as the expected selector/panel-absence RED.

After Task 2, run the focused UI, presentation-state, fixture-host, and lower-layer selectors with a unique `-derivedDataPath` and `-resultBundlePath`; confirm every requested selector executed once with no failure or skip. Preserve the `xcresulttool` summary and test list, inspect all retained UI-test attachments for recovery-key or document-metadata disclosure (the VD2-07 tests add no screenshot or text attachment), and record the per-selector outcome. Then build the normal app and the UI test host using the configured Debug signing identity, verify the app, host, and test-bundle signatures with `codesign --verify --deep --strict`, record `codesign -dvv`, inspect the project structural diff, and run `git diff --check`. Never use `CODE_SIGNING_ALLOWED=NO` for this work.
