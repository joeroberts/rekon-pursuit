...
must remain usable at a compact desktop window (900 × 640 points).
...
2. Refactor `RekonPursuit/ContentView.swift` so it owns the existing
   ...
   hidden destination. The existing native CSV `NSOpenPanel` path remains
   unchanged.
3. Update `RekonPursuit/BootstrapApp.swift` only as necessary to set a useful
   desktop default size and minimum content size. Update
...
4. Update `RekonPursuitUITests/RekonPursuitUITests.swift` before the refactor:
   ...
   on launch. Add the smallest UI smoke that creates a workspace, saves an
   existing opportunity, changes destinations, and observes its refreshed
   visible count/record without relaunch. Keep deterministic local test data.
...
- Creating a workspace, saving/editing an existing opportunity, importing a
  CSV preview, and navigating between destinations visibly refreshes the
  shell without relaunch.
...
2. Perform one isolated manual smoke at 900 × 640: first launch → create
   workspace → add a synthetic opportunity → select Pipeline and Activity &
   AI → select Import CSV and open/cancel the native chooser. Verify visible
   immediate refresh and no clipped primary action.
