...
- [ ] Write a lifecycle traceability matrix before changing any decision text. Map every ADR acceptance criterion to fixture IDs and evidence: `WS-EMPTY-001`, `WS-READONLY-001`, `MIGRATE-*`, `BACKUP-*`, `RESTORE-*`, `RECOVERY-MISSING-001`, `DELETE-LOGICAL-001`, `BACKUP-RETENTION-001`, `BACKUP-PURGE-001`, `EXPORT-ENCRYPTED-001`, `EXPORT-UNENCRYPTED-001`, and `EXPORT-CANCELLED-001`.
...
- [ ] Add the M1 fixture set and builders first: `WS-EMPTY-001`, `WS-CORE-001`, `WS-READONLY-001`, `MIGRATE-NMINUS1-001`, `MIGRATE-FAIL-001`, `DB-CORRUPT-001`, all `BACKUP-*`, all `RESTORE-*`, `RECOVERY-MISSING-001`, `DELETE-LOGICAL-001`, `BACKUP-RETENTION-001`, `BACKUP-PURGE-001`, `EXPORT-ENCRYPTED-001`, `EXPORT-UNENCRYPTED-001`, `EXPORT-CANCELLED-001`, and `RECON-OFFLINE-001`.
...
- [ ] Write failing lifecycle/recovery tests with `WS-READONLY-001`, `DB-CORRUPT-001`, `RECOVERY-MISSING-001`, `BACKUP-VALID-001`, `BACKUP-CORRUPT-001`, `BACKUP-SWAP-001`, `RESTORE-KEYCHAIN-001`, `RESTORE-CLEANMAC-001`, `DELETE-LOGICAL-001`, `BACKUP-RETENTION-001`, `BACKUP-PURGE-001`, `EXPORT-UNENCRYPTED-001`, and `EXPORT-CANCELLED-001`. Tests must reflect the M0-2 contract exactly, including restore to a new workspace and no partial overwrite.
...
- [ ] Evidence bundle `M1-QA-01` through `M1-QA-09`: test reports, fixture IDs, clean toolchain/CI logs, entitlement/architecture inspection, dependency/secret scan, macOS-14.0 clean-device smoke, and redacted recovery/diagnostic output.
...
- Wrong/missing/locked/denied Keychain, corrupt workspace, failed migration, missing recovery secret, tampered/swap backup, interrupted purge, canceled export, and offline execution each produce the approved safe state and recovery guidance—never plaintext fallback, silent loss, automatic external request, partial overwrite, or empty replacement workspace.
...
- A separate Code Reviewer, QA/Test verifier, Architect, and Security/Privacy verifier all approve. TPM/Delivery Manager record `M1` acceptance only after all `M1-QA-01` through `M1-QA-09` evidence is present and no P0/P1 trust, loss, audit, accessibility, or compatibility defect remains.
