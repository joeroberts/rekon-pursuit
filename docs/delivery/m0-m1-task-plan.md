...
The following remain blocked until their roadmap gates: contacts/interactions (M3); CSV import (M4); reconciliation beyond `RECON-OFFLINE-001` fixture protection (M5); documents/attachments (M5/Phase 2c); AI route selection, cloud/local inference, AI cost entries, and budgets (M6); Gmail/Calendar OAuth and mutations (M7); employer research and document processing (M8); interview and offer decision support (M9). A task that adds any of these must be rejected or returned to its roadmap gate.
