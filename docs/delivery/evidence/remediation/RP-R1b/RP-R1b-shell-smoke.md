...
| UI navigation checks | `xcodebuild test -project RekonPursuit.xcodeproj -scheme RekonPursuit -destination 'platform=macOS,arch=arm64' -only-testing:RekonPursuitUITests` | Passed: default Needs Attention/sidebar/workspace-state and Contacts navigation. |
