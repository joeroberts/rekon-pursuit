...
| Post-MVP refinement | **Active delivery queue** — VD2-01 through VD2-05 are accepted. VD2-06 is active at its Task 1 test foundation only; Task 2 is blocked pending fresh QA evidence review. VD2-07 and VD2-08 remain Backlog and blocked from release. |
...
| `VD2-06` | Contacts master/detail redesign | **In progress — Task 1 test foundation only.** Delivery released only signed `contacts`/`contacts-empty` fixture, low-layer no-write/failure/audit, and Contacts-reaching presentation-RED evidence. Task 2 is blocked until fresh independent QA accepts Task 1 source/evidence, then Architecture, TPM, and Delivery approve continuation. [Delivery gate](../../.superpowers/sdd/2026-07-31-vd206-contacts-master-detail/preimplementation-delivery-gate.md). |
| `VD2-07` | Settings information architecture | Backlog and blocked from release — requires `VD2-02` accepted plus explicit product-owner acceptance of a normally signed Debug VD2-06 build; VD2-06 is only at Task 1. |
| `VD2-08` | Visual QA and accessibility acceptance | Backlog and blocked from release — requires `VD2-03` through `VD2-07` accepted; broad acceptance is out of the VD2-06 Task 1 scope. |
