...
| `VD2-07b` | Shared form-input visual alignment | **Backlog.** Align every app-owned form input with the approved dark inset field system: Contacts editor/search, Pipeline opportunity forms, and protected-export dialogs. Preserve bindings, validation, persistence, behavior, and native save/open panels. Accessibility remediation remains `VD2-08`; no implementation is released. See the [task brief](task-briefs/VD2-07b-shared-form-input-alignment.md). |
...
nine children, including `VD2-08`, are accepted and the product owner accepts
