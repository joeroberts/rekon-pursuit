...
| `MIGRATE-FAIL-001` | Pre-migration workspace plus injected migration failure at a named transaction step. | Rollback, preserved pre-migration backup, actionable version/error state. |
...
| `RECOVERY-MISSING-001` | Backup with required recovery material absent or denied. | Clear unrecoverable-loss guidance; never plaintext fallback or empty workspace creation. |
| `DELETE-LOGICAL-001` | `WS-CORE-001` after a user deletes the opportunity at a fixed time. | Record disappears from normal views/FTS, tombstone/activity remains, related deletion rules match the ADR. |
| `BACKUP-RETENTION-001` | Backup containing `DELETE-LOGICAL-001` data at day 0, day 29, and day 30 relative to fixed clock. | Default 30-day retention, visible expiry, deterministic eligibility calculation. |
| `BACKUP-PURGE-001` | Two retained backups with deleted and non-deleted records. | Confirmed purge removes only eligible deleted data from retained backups; cancellation/no confirmation changes nothing. |
...
| `EXPORT-UNENCRYPTED-001` | User-approved plaintext export with fixed filename/destination choice. | Exact warning and destination review precede write; export is complete and marked outside app controls. |
...
| `M0-QA-03` | Local-data lifecycle ADR resolves backup envelope, recovery trust, restore/re-wrap, tombstone/search behavior, 30-day backup retention, purge semantics, and unencrypted-export warning. | Architect | Security/privacy verifier and QA/test agent |
...
| `M1-QA-04` | `WS-READONLY-001`, `DB-CORRUPT-001`, `RECOVERY-MISSING-001` | Read-only/corrupt/key-unavailable states block mutations, show recovery guidance, and never replace the workspace with a blank one. | Implementer / QA verifier + Security/privacy verifier |
...
| `M1-QA-06` | `DELETE-LOGICAL-001`, `BACKUP-RETENTION-001`, `BACKUP-PURGE-001`, `EXPORT-UNENCRYPTED-001`, `EXPORT-CANCELLED-001` | Approved deletion, retention, purge, and export warning semantics are exact and deterministic. | Implementer / QA verifier |
