...
| Backup class | M5 supports exactly one retained class: an opt-in portable/recoverable encrypted backup. It has `created_at`, `expires_at = created_at + 30 × 24 hours`, verification state, and a recovery envelope. The M1 MVP contains no portable backup, restore, purge, or export command. |
...
| Secret boundary | Database keys, OAuth tokens, recovery secrets, plaintext backup keys, full paths, and raw export payloads never enter backup manifests/envelopes, exports, activity records, ledger entries, diagnostics, or test fixtures. Raw deleted payloads never enter those metadata/diagnostic/export surfaces. An encrypted recoverable archive created **before** deletion may retain its original deleted payload until its displayed expiry or verified purge, as ADR-001 requires; a replacement created by purge excludes it. Recovery enrollment performs one user-mediated display/re-entry. The sole clipboard exception is an explicit **Copy recovery key** action with a clear warning that other apps or clipboard history can retain the value; the app never copies or clears it automatically, and never writes it to a file. |
...
| `EnrollRecoveryKey` (M5) | One-time user-mediated reveal, optional explicit warned clipboard copy, re-entry verification, recovery envelope/trust binding, then enables portable backup creation. | Cancel, mismatch, Keychain failure, or disk interruption creates no recoverable backup and leaves enrollment disabled. There is no reset, escrow, support override, or file export; clipboard writes are only user-initiated and warned, never automatic. |
...
| --- | --- |
| `CreateRecoverableBackup` (M5) | Requires verified enrollment. Writes an authenticated archive, manifest, and recovery envelope to temporary names; verifies the complete replacement before atomic promotion. The manifest binds only archive ID, `created_at`, and the canonical snapshot hash. Format/suite, `expires_at`, payload checksum, recovery envelope, and verification-key identity are bound directly by the signed outer-header commitment, avoiding any manifest/envelope/ciphertext dependency cycle. Privacy-minimized deleted-material inventory remains inside the authenticated encrypted snapshot. Neither surface records a secret or plaintext key. |
| `ExpireBackup` (M5) | At `now >= expires_at`, queues removal by the same verify/remove discipline. User-visible status remains `expired_pending_removal` until removal finishes; physical overwrite is best-effort only. |
| `PurgeDeletedBackups` (M5) | On confirmation, captures an immutable scope of eligible backup IDs and their manifest versions (`scope_cutoff`). It blocks/queues new backup creation and manifest mutation until completion, cancellation, or failure. For each scoped backup, it regenerates a replacement excluding eligible logically deleted content, verifies replacement/authentication/bindings, atomically promotes it, then removes its predecessor. A predecessor is never removed before its replacement verifies. |
...
| `RestoreBackup` (M5) | Always restores to a new workspace directory. For the released v1 portable format, clean-Mac authority is recovery-key possession plus complete authenticated package verification (header/signature/fingerprint consistency, envelope unwrap, manifest, archive authentication, checksums, and compatibility), followed by explicit confirmation of the verified archive ID, creation time, and signing-key fingerprint. V1 does not claim independent source provenance. When a local catalogue row is available, its archive ID and signing-key fingerprint must match the package. A stronger recovery-secret provenance binding may be added only by a future archive-format version; it is not a v1 requirement. Corrupt archive/envelope/manifest, swapped archive/envelope, substituted signing key, or local-catalogue ID/fingerprint mismatch rejects without partial restore/overwrite. |
| Post-clean-Mac restore (M5) | Re-wraps to a fresh workspace root/database key and fresh backup-signing keypair; it does not import OAuth tokens or reuse the recovery secret as an operating key. It requires new recovery enrollment before future recoverable backups. Any stronger portable source-provenance binding applies only after a future versioned archive-format migration, never retroactively to v1. |

### Export confirmation

M5 implements the `StartExport` → `ReviewExport` → `ConfirmExport` state machine. It defaults to encrypted export. The confirmation fingerprint is a canonical hash of export type, selected categories, exact filename, and canonical destination. Any change to those values invalidates the prior confirmation and returns to review. Unencrypted export requires a distinct warning that it is outside Rekon Pursuit encryption/deletion controls. Cancellation writes no file and no completion event. A completion event contains only export type, selected categories, destination class, confirmation fingerprint, and outcome—not full paths or contents. M1 presents no export action and makes that limitation explicit.
...
| Workspace unavailable/corrupt never becomes a blank replacement | §2 `OpenWorkspace` | `WS-READONLY-001`, `DB-CORRUPT-001` | Mutations are blocked, existing state is retained, and recovery guidance is shown. No plaintext fallback or silently created empty workspace. | QA + Security |
...
| Recovery setup has no reset/escrow (M5) | §2 Workspace and Keychain | `RECOVERY-ENROLL-001`, `RECOVERY-MISSING-001` cases `locked`, `denied`, `missing`, `reentry_mismatch`, `cancelled` | No backup/enrollment on failure; no blank workspace, plaintext fallback, secret logging, file write, or reset route. | Security + QA |
| 30-day expiry from backup creation and deletion disclosure (M5) | §1 / §2 backup | `BACKUP-RETENTION-001` cases `created_before_delete_day0`, `day29`, `day30`, `expired_pending_removal` | `expires_at` equals creation + 30 days, not deletion + 30; prior backup remains disclosed until expiry/purge. | QA |
| Purge is destructive, verified, truthful (M5) | §2 Purge | `BACKUP-PURGE-001` cases `confirm`, `cancel`, `multi_backup_failure`, `predecessor_removal_failure`, `interruption_relaunch`, `retry`, `concurrent_create` | No predecessor removal before verified replacement; predecessor-removal failure remains `incomplete_retryable`/`blocked`; per-backup durable status; no false complete result. | QA + Security |
| Restore uses new workspace and rejects tampering/substitution (M5) | §2 Restore | `BACKUP-VALID-001`, `BACKUP-CORRUPT-001` variants, `BACKUP-SWAP-001` variants, `RESTORE-KEYCHAIN-001` cases, `RESTORE-CLEANMAC-001` cases | New workspace only; no overwrite/partial promotion; v1 clean-Mac authority is recovery-key possession plus authenticated package verification and explicit confirmation, while same-Mac catalogue ID/fingerprint mismatches reject. Stronger provenance binding is future-version-only. | Architect + QA + Security |
| Encrypted default / warned unencrypted export (M5) | §2 Export | `EXPORT-ENCRYPTED-001`, `EXPORT-UNENCRYPTED-001` cases `warning`, `filename_change`, `destination_change`, `category_change`, `EXPORT-CANCELLED-001` | Encryption is default. Any fingerprint input change invalidates confirmation; cancelled export has no file/event. | QA + Security |
