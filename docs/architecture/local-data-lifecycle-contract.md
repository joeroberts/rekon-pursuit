...
| --- | --- |
| `CreateRecoverableBackup` (M5) | Requires verified enrollment. Writes an authenticated archive, manifest, and recovery envelope to temporary names; verifies the complete replacement before atomic promotion. A manifest records backup/workspace IDs, `created_at`, calculated `expires_at`, format/schema/app/encryption versions, archive and blob checksums, recovery-envelope hash, verification-key ID, and deleted-material inventory summary. It never records a secret or plaintext key. |
