...
| Secret boundary | Database keys, OAuth tokens, recovery secrets, plaintext backup keys, raw deleted payloads, full paths, and raw export payloads never enter backup archives/manifests, exports, activity records, ledger entries, diagnostics, or test fixtures. Recovery enrollment performs one user-mediated display/re-entry; it is not an app-written export or clipboard/file handoff. |
...
| Purge is destructive, verified, truthful | §2 Purge | `BACKUP-PURGE-001` cases `confirm`, `cancel`, `multi_backup_failure`, `interruption_relaunch`, `retry`, `concurrent_create` | No predecessor removal before verified replacement; per-backup durable status; no false complete result. | QA + Security |
...
| Sensitive material never leaves protected boundary | §1 Secret boundary; §2 all operations | `LIFECYCLE-REDACTION-001` across events, diagnostics, manifests, envelopes, export ledger metadata | Redaction scans find no key/token/secret/raw deleted payload/full path/raw export content. | Security |
