...
- Default retention periods, secure-delete behavior, and export/delete semantics.
- Backup recovery-key policy. **Required release policy assumption:** backup recovery is opt-in; the app never escrows a recovery key, transmits it, or creates a recoverable backup until the user has explicitly completed recovery-key setup and acknowledged that an unrecorded key cannot be recovered.
...
Portable/recoverable backups remain unavailable until the user opts in to recovery-key setup. During setup, the app generates a high-entropy, user-controlled recovery secret, shows it once for offline storage (and, when supported, permits export to a user-selected encrypted password manager/file), and requires a re-entry verification before recoverable backup creation. The secret is never stored in the workspace database, Keychain, diagnostics, activity summary, or product-controlled service. A user may choose a passphrase-derived recovery secret only if the KDF is Argon2id (or a platform-approved equivalent) with versioned, stored parameters and a calibrated work factor; the app must not silently weaken those parameters.
...
- References use foreign keys. Immutable/audit tables never cascade-delete; they preserve a nullable/redacted subject reference and a display snapshot when a subject is deleted.
...
| `import_batches`, `import_rows`, `duplicate_candidates`, `import_decisions` | original CSV blob, mapping JSON, row validation, candidate/rationale, decision (`create`, `update_selected_fields`, `keep_separate`, `skip`), result IDs; each row needs an explicit terminal decision before batch completion |
...
- Create an encrypted on-device backup before migrations and after a configurable number of material mutations/at app quit; retain according to user policy. Before recovery-key setup, these are local-Keychain-only resilience backups and the UI must label them as non-portable/non-recoverable after Keychain loss. Recoverable backups are created only after the verified opt-in flow in the recovery-key strategy. Backups are atomic SQLite backups plus a signed manifest containing schema version, database checksum, blob inventory/checksums, app version, encryption version, and recovery-envelope hash.
- Restore always targets a newly created workspace directory, verifies signature, envelope/archive authentication, manifest/checksums, and schema compatibility, then asks the user to switch workspaces. Never overwrite the active workspace in place. A restore cannot enter the switch-workspace gate until all verification succeeds.
- Export creates a user-selected encrypted archive or sanitized CSV/JSON plus linked copies where selected. Exports exclude OAuth tokens and database keys.
- Delete is a confirmation flow. It first writes an export/backup option, revokes integrations if requested, marks local records for deletion, removes blobs/caches, and deletes Keychain items only after filesystem/database removal succeeds. The activity ledger records deletion metadata until its own removal. Secure-delete guarantees depend on APFS/SSD behavior and must be described as best-effort.
...
Search is local-only and uses an encrypted FTS5 projection (or encrypted token index) over permitted ledger fields: event type, normalized summary tokens, actor, subject display snapshot, provider/model, route, failure class, date, and stable IDs. It never indexes raw prompts, email/calendar bodies, unredacted before/after patches, recovery material, tokens, or diagnostic captures. `AuditService` exposes deterministic filters for date range, event type, actor, subject/opportunity, route, provider/model, execution/result state, confirmation ID, correlation ID, and a free-text query; result detail loads the authoritative event and applies `audit_redactions` at read time. Retention applies to the event, its query projection, and linked AI/output/payload references together: a redaction/deletion event removes searchable content and access to the encrypted artifact, while preserving the minimum permitted immutable tombstone (ID, time, type, redaction reason, and subject display snapshot) for ledger integrity. Default retention and legal/user overrides are policy values; no payload artifact may outlive its declared retention reason.
...
The import wizard persists a batch at selection and follows the UX flow exactly: `map_columns → validate_rows → decide_duplicates → commit → report`. Validation is deterministic/offline and reports row-level errors. Duplicate candidates are advisory, scored with an explainable local rule set (canonical URL, employer/title similarity, and optional date/location); no candidate is auto-merged.

Every duplicate row requires `create`, `update_selected_fields` (show field-by-field diff), `keep_separate`, or `skip`; the user must select exact fields for an update. Batch commit creates records, decisions, one activity event per materially changed row, and a batch summary atomically. A reversible import means an `Undo import` command may tombstone only rows/field revisions still owned solely by that batch; it never deletes later user edits or shared linked records. The report displays created, updated, skipped, errors, duplicate rationale, and undo eligibility.
...
Use deterministic clocks, random seeds, fixture provider responses, and an injectable filesystem/Keychain/HTTP/XPC layer. CI runs formatter/linter, unit/database/contract tests, security dependency scan, migration compatibility tests, clean-macOS-14.0 build/launch smoke tests, and helper-entitlement inspection. Developer ID signing, notarization, stapling, and DMG smoke tests are M5 release-gate evidence. A release gate requires encrypted backup/restore smoke tests for both available recovery paths (including clean-Mac portable trust verification), no unresolved migration or baseline/dependency-matrix checks, and manual verification that full-cloud and external mutations cannot execute without their confirmation sheets or their authenticated canonical payload artifacts.
