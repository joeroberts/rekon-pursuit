...
Portable archive v1 authenticates its package with its recovery-key envelope, signed header, manifest, and payload bindings. On a clean Mac without a local catalogue, recovery-key possession plus complete v1 package verification is the authority to restore; the app shows the verified archive ID, creation time, and signing-key fingerprint and requires explicit confirmation before creating a new workspace. This is not an independent source-provenance claim: a party that has the recovery key can produce a separately signed v1 package. When a local catalogue is available, its archive ID and signing-key fingerprint must match the verified package. A future, versioned archive format may add a recovery-secret-derived provenance binding to a known verification key; it must not be inferred or claimed for v1.

Restore creates a new workspace only after it verifies the applicable v1 package authentication, envelope binding, archive checksums, canonical snapshot, and compatibility. If the old Keychain is available, it also requires the matching local catalogue identity; otherwise the user supplies and confirms the recovery key for the explicit clean-Mac v1 flow. A successful restore generates a fresh workspace root/database key **and a fresh backup-signing keypair**; it does not import old OAuth tokens or reuse the recovery secret as an operating key. The user must generate and verify a replacement recovery secret/envelope before enabling subsequent recoverable backups. Changing, rotating, or disabling the recovery secret creates new envelopes for retained backups only after re-authentication; old envelopes are securely removed according to retention policy. If both the Keychain material and recovery secret are lost, backup data is intentionally unrecoverable: show clear loss guidance, allow deletion of inaccessible backups, and never offer a password-reset, support override, or empty-workspace replacement as recovery.
...
| End-to-end | local-only daily loop; re-import with row decisions; confirmed closure; Gmail selected-thread accept/reject suppression; high/low-confidence Gmail response assessment with manual review and explicit follow-up acceptance; email draft then exact confirmation plus timeout/crash/relaunch unknown-outcome reconciliation; Calendar conflict review; each AI route and no-fallback proof; export/restore with local-Keychain catalogue identity matching and a clean-Mac/no-prior-Keychain v1 recovery-key restore that verifies the authenticated package, requires explicit identity confirmation, re-wraps fresh operating keys, and rejects corrupted/swapped archive, envelope, manifest, or signing-key substitution; unrecoverable-loss guidance |
