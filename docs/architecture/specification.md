...
- The only retained backup class in M1 is the ADR-001 portable/recoverable backup: it is opt-in, encrypted/authenticated, and expires exactly 30 days after `created_at`. Before recovery-key enrollment, migration protection may create only an ephemeral verified rollback snapshot within the migration transaction; it is destroyed on successful migration or retained only long enough to preserve a failed migration's recovery path. It is not shown as a backup, cannot be restored/exported independently, has no 30-day retention window, and cannot contain logically deleted data from a prior completed workspace state. No scheduled or app-quit local-Keychain-only resilience backup is permitted in M1. Backups are atomic SQLite backups plus a signed manifest containing schema version, database checksum, blob inventory/checksums, app version, encryption version, and recovery-envelope hash.
