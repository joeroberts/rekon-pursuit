...
checksum and verified by re-entry. The app never writes it to a file. The user
may explicitly choose **Copy recovery key** after a clear warning that other
apps or clipboard history can retain it; the app never copies or clears the
clipboard automatically. The app retains only a one-way enrollment fingerprint,
so every portable-backup creation requires the user to re-enter the recovery
...
6. Automatic expiry may remove a backup only when both catalogue predicates hold: `storage_class == "managed"` and `managed_relative_path` is the generated canonical `<lowercase archive UUID>.rekonarchive` leaf beneath the workspace-private `portable-archives` root. Every other row—including pre-v28 migrated rows, which are `external` rather than a third class, and external/user-selected rows—is not a destructive target: expiry never resolves its external bookmark, opens, renames, or deletes it and records the durable manual-removal or blocked outcome. A verified managed target follows the same best-effort deletion process. APFS/SSD media prevents a guarantee of physical overwrite; the product must describe physical secure deletion as best effort.
