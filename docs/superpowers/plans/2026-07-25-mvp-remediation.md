...
**Scope:** Replace hidden first-run creation with an explicit default-page workspace gate. Model database artifacts × no key, primary key only, pending key only, and both keys, plus corrupt, locked, denied, unavailable, ready, retry, and recovery states. Any database/key/journal combination outside the one verified ready pair is recovery-required: open/retry never deletes, promotes, ignores, or overwrites a key. Create is unavailable while any database artifact, journal, or either Keychain key exists; recovery copy explains existing material is retained and offers no reset/replacement. Creation writes a non-secret durable journal with attempt ID and phase (`staging`, `pending-written`, `database-promoted`, `primary-promoted`, `cleanup-pending`). Cleanup may remove only artifacts created before commit; it never deletes a database/key pair after database promotion or a reopen error. Derive the Keychain service from immutable compiled bundle configuration, preserving `com.rekonlabs.RekonPursuit.workspace` for production; never route it through environment variables or user preferences. Enable CSV selection only after ready state and show why otherwise.

**Acceptance:** Focused fixtures for no-database/primary-key, no-database/pending-key, database/no-key, database/primary-key, database/pending-key, database/both-keys, corrupt database, locked, and denied states preserve original artifacts and block replacement as applicable. Inject one failure at each boundary—staging database close, pending-key write, database promotion, primary-key promotion, pending-key cleanup, and final reopen—and assert the exact journal/artifact state is retained; in particular, an error after database promotion/final reopen preserves the committed database and primary key unchanged. A fresh-launch smoke uses a distinct sandboxed temporary bundle, app-data container, and compiled Keychain service; entitlement inspection proves App Sandbox and evidence records only the redacted temporary namespace ID and temporary-container assertions. The smoke never reads or modifies the production workspace or Keychain namespace. Absolute paths, Keychain item/account metadata, keys, and fixture content are prohibited from the journal, evidence, diagnostics, activity, test output, and displayed copy. Create Workspace succeeds; pipeline and CSV entry point become reachable without relaunch; a synthetic fixture preview appears; failure/retry paths do not destroy existing data.
