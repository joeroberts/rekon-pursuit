...
final class WorkspaceStore {
    ...
    func create(_ command: CreateOpportunity) throws -> Opportunity {
        ...
        return try synchronized {
            let opportunity = Opportunity(id: nextIdentifier(), title: title, company: company, createdAt: now)
            ...
            try database.transaction {
                try database.execute(
                    "INSERT INTO opportunities (id, title, company, created_at) VALUES (?, ?, ?, ?)",
                    values: [.text(opportunity.id), .text(opportunity.title), .text(opportunity.company), .real(opportunity.createdAt.timeIntervalSince1970)]
                )
    ...
    func opportunities() throws -> [Opportunity] {
        try synchronized {
            try database.rows("SELECT id, title, company, created_at FROM opportunities ORDER BY created_at, id").map(opportunity(from:))
        }
    }
    ...
    private func opportunity(from row: [DatabaseValue]) throws -> Opportunity {
        guard row.count == 4,
              ...
              case let .text(company) = row[2], case let .real(createdAt) = row[3] else {
            throw WorkspaceStoreError.unexpectedDatabaseValue
        }
        return Opportunity(id: id, title: title, company: company, createdAt: Date(timeIntervalSince1970: createdAt))
    }
